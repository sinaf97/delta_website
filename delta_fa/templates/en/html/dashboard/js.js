...
function reset_modal(user){
  ...
  <form id="edit-user-form" action="/en/dashboard/users/edit_user" class="form" style="border:1px solid grey;border-radius:5px;margin-bottom:10%">\
      <table>\
      <tr>\
      <td style="width:80%">\
         <div style="float:left" class="form-group col-md-6">\
           <label for="fn">Firstname</label>\
           <input type="text" name="fn" class="form-control" value="'+user.fields.latin_first_name+'" disabled>\
           <label for="ln">Lastname</label>\
           <input type="text" name="ln" class="form-control" value="'+user.fields.latin_last_name+'" disabled>\
         </div>\
         <div style="float:left" class="form-group col-md-6">\
         </div>\
         </td>\
         ...
         <div class="form-row">\
           ...
           <div class="form-row">\
             ...
             <div class="form-group col-md-4">\
               ...
               <input type="text" name="rl" class="form-control" value="'+user.fields.latin_role+'" disabled>\
             </div>\
           </div>\
           <div class="form-row">\
  ...
  $(".modal-footer").append('<a type="button" class="eClass btn btn-danger" style="color:white" href="/en/dashboard/users/'+user.fields.username+'/edit_user" id="edit">Edit Profile</a>')
  ready()
  $('#modal').modal();
}
...
function reset_course_modal(course,teacher,term,group){
  ...
  $('.modal-header').append('<a class="cClass" href="/en/dashboard/course/'+ order({'code':course.fields.code,'group':group,'term':term}) +'"><h1>'+course.fields.course_name+'</h1></a>\
      ...
      <div style="margin:10px">\
       <div class="form-row">\
         <div class="form-group col-md-4">\
           ...
           <input type="text" name="fn" class="form-control" value="'+teacher.fields.latin_first_name+' '+teacher.fields.latin_last_name+'" disabled>\
         </div>\
         <div class="form-group col-md-4">\
         </div>\
         <div class="form-group col-md-4">\
         <label for="ln" style="color:white">!</label>\
           <input name="ln" id="to-be-white" type="button" class="aClass btn btn-primary form-control" value="Teacher profile" href="/en/dashboard/users/'+teacher.fields.username+'">\
         </div>\
       </div>\
       </div>\
     </form>\
...
function profile(addressValue){
  $.ajax({
    ...
    success: function (data) {
      ...
      if(user.fields.latin_role == "Teacher")
      // console.log("teacher");
      teacher_modal_creator(user,data.courses)
      else if(user.fields.latin_role == "Student")
...
function add_courses(courses){
  ...
  for(var i=0;i<sina.length; i++)
    $('#term'+sina[i]['term']['season']+sina[i]['term']['part']).append('<li style="font-size:12px"><a id="'+sina[i].term.year+sina[i].term.season+sina[i].term.part+sina[i].code+'" class="cClass nav-link" href="/en/dashboard/course/'+ order(sina[i]) +'">'+ sina[i].course_name +'</a></li>')
}
function add_students(student,count){
  ...
  '<tr>\
    <td>'+count+'</td>\
    <td style="width:30%">'+user.fields.latin_first_name+' '+user.fields.latin_last_name+'</td>\
    ...
    <td style="width:10%"><a class="aClass" href="/en/dashboard/users/'+user.fields.username+'">Show</a></td>\
  </tr>')
}
...
function fill_courses(course,count){
    ...
    $('#course-list').append(
      ...
      <td style="width:20%"><a class="cClass" href="/en/dashboard/course/'+ order(course) +'">Show</a></td>\
      </tr>')
    }
function fill_user(user,count){
  ...
  $('#user-list').append(
    ...
    <td style="width:30%">'+user.fields.latin_first_name+' '+user.fields.latin_last_name+'</td>\
    <td style="width:20%%">'+user.fields.username+'</td>\
    <td style="width:15%">'+user.fields.latin_role+'</td>\
    <td style="width:29%">'+user.fields.mobile+'</td>\
    <td style="width:10%">'+(user.fields.is_active?'Active':'Deactive')+'</td>\
...
function teacher_courses(sina){
  ...
  for(var i=0;i<sina.length; i++)
    $('#term'+sina[i]['term']['season']+sina[i]['term']['part']).append('<li><a class="nav-link" href="/en/dashboard/courses/'+ order(sina[i]) +'">'+ sina[i].course_name +'</a></li>')
...
function show_mail(id,mail){
  ...
  $('#reply').attr('href','/en/dashboard/mail/reply/'+mail.origin.username)
  seen(mail.id)
}
function seen(id){
    $.ajax({
      url: '/en/dashboard/mail/inbox/seen',
      data: {
        'id': id
      },
      method:'GET',
